...
vLLM configures only its own `vllm` logger, so the plugin attaches a stderr handler to the `vllm_lens` logger when it has none. Set `VLLM_LENS_LOG_LEVEL` to change the level (default `INFO`; a value that is not a level name is ignored):
