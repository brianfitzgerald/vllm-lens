...
vLLM configures only its own `vllm` logger, so the plugin attaches a stderr handler to the `vllm_lens` logger when it has none, and turns off propagation for it, as vLLM does for `vllm`. With `VLLM_LENS_DISABLE=1` the plugin does not change logging. Set `VLLM_LENS_LOG_LEVEL` to change the level (default `INFO`; a value that is not a level name is ignored):
...
- Each layer in the list adds `max_num_batched_tokens x hidden_size` values to the graph output pool, so name only the layers you read.
...
Before each forward pass the plugin writes the rows of every steered request into buffers, and an op inside the graph adds them to the layer output. Zero rows change nothing, so requests with and without steering share a batch and the graph has no extra boundary. `apply_steering_vectors` does not change: `scale`, `norm_match`, `position_indices` and several vectors on one layer work as in eager mode. Limits:
