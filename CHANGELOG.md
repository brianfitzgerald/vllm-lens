...
- Examples: Added a test that runs the activation oracle example in eager mode and under CUDA graphs (`VLLM_LENS_CAPTURE_LAYERS`, `VLLM_LENS_STEER_LAYERS`). The oracle names the three philosophers in both modes, and names none when the steering scale is 0.
