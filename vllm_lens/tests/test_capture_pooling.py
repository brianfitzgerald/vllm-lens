...
async def _generate_async(engine: AsyncLLMEngine, request_id: str, pool: str | None):
    """Run ``PROMPT`` with capture at ``LAYERS`` and return the final output."""
    extra_args: dict = {"output_residual_stream": LAYERS}
    if pool is not None:
        extra_args["output_residual_stream_pool"] = pool
    params = SamplingParams(temperature=0.0, max_tokens=3, extra_args=extra_args)
    final = None
...
async def test_pooled_capture_on_the_async_engine(async_engine, pool):
    """The async path returns the same pooling of its own full capture."""
    full_out = await _generate_async(async_engine, f"full-{pool}", None)
    pooled_out = await _generate_async(async_engine, f"pool-{pool}", pool)
    full = full_out.activations["residual_stream"]  # type: ignore[reportAttributeAccessIssue]
    pooled = pooled_out.activations["residual_stream"]  # type: ignore[reportAttributeAccessIssue]
    prompt_rows = full[:, : len(full_out.prompt_token_ids)].float()
    expected = (
        prompt_rows[:, -1:] if pool == "last" else prompt_rows.mean(1, keepdim=True)
    )
    torch.testing.assert_close(pooled.float(), expected, atol=1e-2, rtol=1e-2)
...
async def test_unknown_pool_value_raises_on_the_async_engine(async_engine):
    """The async path rejects a value other than ``last`` or ``mean``."""
    with pytest.raises(ValueError, match="output_residual_stream_pool"):
        await _generate_async(async_engine, "pool-max", "max")
...
@pytest.mark.parametrize(
    ("pool", "step"), [("last", (0, 4)), ("mean", (10, 1))], ids=["last", "mean"]
)
def test_a_step_without_a_new_prompt_row_makes_no_entry(pool, step):
    """An early chunk (``last``) or a decode step (``mean``) stores nothing. No GPU."""
    extension = SimpleNamespace(_captured_states={}, _pooled_counts={})
    first, n_rows = step
    _accumulate_pooled(
        extension, "req", LAYER_IDX, torch.zeros(n_rows, 8), first, 10, pool
    )
    assert extension._captured_states == {}
    assert extension._pooled_counts == {}
