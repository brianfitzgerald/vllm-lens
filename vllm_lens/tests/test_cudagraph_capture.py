...
@pytest.mark.parametrize("layers", [[1], True], ids=["unarmed-layer", "all-layers"])
def test_an_unserved_capture_is_rejected(graph_llm, layers):
    """A layer outside the armed list, or "all layers", raises before generation."""
...
def test_pipeline_parallelism_is_refused_at_load():
    ...
    The reason is in the worker log; the parent sees only the failed start.
    """
    with pytest.raises(
        Exception, match="(?i)initialization failed|pipeline parallelism"
    ):
        _make_llm({"VLLM_LENS_CAPTURE_LAYERS": "2"}, pipeline_parallel_size=2)
